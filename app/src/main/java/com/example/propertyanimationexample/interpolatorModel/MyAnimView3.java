package com.example.propertyanimationexample.interpolatorModel;
...
import com.example.propertyanimationexample.objectAnimatorModel.ColorEvaluator;
...
public class MyAnimView3 extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (currentPoint == null) {
            currentPoint = new Point((float)getWidth()/2, RADIUS);
            drawCircle(canvas);
            startAnimation();
        } else {
            drawCircle(canvas);
    ...
    private void startAnimation() {
        Point startPoint = new Point((float)getWidth()/2, RADIUS);
        Point endPoint = new Point((float)getWidth()/2, getHeight() - RADIUS);
